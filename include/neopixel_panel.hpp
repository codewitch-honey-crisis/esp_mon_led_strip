...
class neopixel_panel {
    ...
    size_t compute_index(gfx::point16 location) const {
        ...
        x = location.x;
        if(m_snake_layout && m_swap_xy==(location.y&1)) x = m_width-x-1;
        return location.y*m_width+x;
    }
public:
    using pixel_type = gfx::rgbw_pixel<32>;
    using caps = gfx::gfx_caps<false,false,false,false,true,true,false>;
    neopixel_panel(led_strip_type& leds,uint16_t width=0, bool swap_xy = false, bool snake_layout = false) : m_swap_xy(swap_xy), m_snake_layout(snake_layout), m_width(width==0?(uint16_t)leds.length():width), m_suspend_count(0),m_leds(&leds) {
    ...
    gfx::gfx_result point(gfx::point16 location, pixel_type col) {
        ...
        m_leds->color(compute_index(location),r,g,b,w);
        if(!m_suspend_count) {
            m_leds->update();
        }
        return gfx::gfx_result::success;
    ...
    gfx::gfx_result fill(const gfx::rect16& bounds,pixel_type col) {
        ...
        for(int y = bnds.y1;y<=bnds.y2;++y) {
            for(int x = bnds.x1;x<=bnds.x2;++x) {
                m_leds->color(compute_index(gfx::point16(x,y)),r,g,b,w);
            }
        }
        if(!m_suspend_count) {
            m_leds->update();
